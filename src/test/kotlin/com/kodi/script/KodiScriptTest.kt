...
class KodiScriptTest {
    ...
    fun `test array functions`() {
        ...
        // Slice and index
        assertEquals(1.0, KodiScript.run("numbers[1]", vars).value)
        val sliceResult = KodiScript.run("slice(numbers, 4, 5)", vars)
        assertFalse(sliceResult.hasErrors)
        @Suppress("UNCHECKED_CAST") val slice = sliceResult.value as List<Double>
        assertEquals(5.0, slice.last())
